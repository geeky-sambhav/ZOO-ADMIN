  ...
  try {
    ...
    if (currentItem.quantity < requestedQuantity) {
      return NextResponse.json(
        {
          success: false,
          message: `Insufficient quantity. Available: ${currentItem.quantity} ${currentItem.unit}, Requested: ${requestedQuantity} ${currentItem.unit}`,
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully used ${requestedQuantity} ${currentItem.unit}. Remaining: ${inventoryItems[itemIndex].quantity} ${currentItem.unit}`,
